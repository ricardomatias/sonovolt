...
void tick_irq_handler() { sonovolt::rp2040::TempoClock::dirtyTick(); }
...
void TempoClock::update(bool run = true) {
  auto freq =
      static_cast<u32>(time::bpm_to_hz(bpm_) * static_cast<float>(PPQN_));
...
void TempoClock::tick() {
  ...
  // else
  // {
  //     jitter_ = absolute_time_diff_us(previous_tick_us_, get_absolute_time())
  //     - us_per_tick_;
  // }
...
uint64_t TempoClock::getTicks() const { return ticker_; }

u8 TempoClock::getBPM() const { return bpm_; }

bool TempoClock::isRunning() const { return is_running_; }
